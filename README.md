...
- Current visible firmware version: `SignalLab 0.1.1-gpio35scope`.
- Shows live waveform, raw value, rolling min/max/range, rail indicators, capture label, elapsed timer, and simple status.
...
At 115200 baud the firmware prints 50 Hz raw CSV rows:

```csv
siglab,ms,raw,min,max,range,rail_low,rail_high,status,label
```

Command input:

```text
LABEL <text>   update the screen label and CSV label field
...
```text
version,SignalLab 0.1.1-gpio35scope
```
